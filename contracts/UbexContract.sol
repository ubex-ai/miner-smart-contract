...
library ArrayUtil {
  function IndexOf(uint[] storage values, uint value) internal view returns(uint index) {
    uint i = 0;
...
contract UbexContract {
  ...
  function addRequest(uint id, address addr, uint256 amount) public onlyBy(owner) {
    if (isEntity(id)) {
      return false;
    }
